...
function monthTotalInterface(interfaceKind, singleDate, timeCycle){
	...
	if(cinemaId.trim() != ""){
		...
		if(interfaceKind == "mtTicketTotal"){
			/*
			* 这种数据是按日期值升序排序，会导致两个问题：
			* 1、若一个对象的数据个数 <=6 ，当单独显示它时，X轴日期计算会出现日期值重复的问题
			* 2、Highcharts报错：Highcharts error #15: www.highcharts.com/errors/15
			* Highcharts 官网错误码定义如下：
			* Highcharts Error #15
			* Highcharts expects data to be sorted
			* This happens when you are trying to create a line series or a stock chart where the data is not sorted in ascending X order. 
			* For performance reasons, Highcharts does not sort the data, instead it is required that the implementer pre-sorts the data.
			* 
			* 错误码定义分析：
			* 发生这个的原因是，当开发者试图根据在X轴不是按升序排序的数据（实际上返回数据默认按升序排序），创建数据线系列时，
			* (Highcharts)出于性能的考虑，Highcharts 不会对这些数据进行排序，相反的，而是需要开发者预先对这些数据进行排序。
			* 
			* 调试发现：这里所说的排序，实际上是要求X轴的日期值降序排序。接口返回的数据虽然是按日期升序，但是Highcharts需要的是降序数据，
			* 而Highcharts兼顾性能，不会对数据进行排序（降序排序），所以需要开发者自己将数据按日期降序处理后，交给Highcharts去渲染。
			*/
			var tempData2 = { "ver" : "1.0" , "ret" : true , "errcode" : 0 , "errmsg" : "" , "cinemaId" : 486,  
							"data" : [
								{ "filmId" : 2, "filmName" : "何以笙箫默", 
									"data" : [{"showDate" : "2015-05-08" , "sessionCount" : 120 , "sessionCountRate" : 1.5},
											{"showDate" : "2015-05-09" , "sessionCount" : 145 , "sessionCountRate" : 1.3}, 
											{"showDate" : "2015-05-10" , "sessionCount" : 160 , "sessionCountRate" : 1.2},
											{"showDate" : "2015-05-11" , "sessionCount" : 90 , "sessionCountRate" : 1.2}, 
											{"showDate" : "2015-05-12" , "sessionCount" : 90 , "sessionCountRate" : 1.2}, 
											{"showDate" : "2015-05-13" , "sessionCount" : 110 , "sessionCountRate" : 1.1}] 
								} ,
								{ "filmId" : 7, "filmName" : "测试影片1", 
									"data" : [{"showDate" : "2015-05-04" , "sessionCount" : 90 , "sessionCountRate" : 1.9}, 
											{"showDate" : "2015-05-05" , "sessionCount" : 80 , "sessionCountRate" : 1.8}, 
											{"showDate" : "2015-05-06" , "sessionCount" : 100 , "sessionCountRate" : 1.7},
											{"showDate" : "2015-05-07" , "sessionCount" : 105 , "sessionCountRate" : 1.6},
											{"showDate" : "2015-05-08" , "sessionCount" : 120 , "sessionCountRate" : 1.5},
											{"showDate" : "2015-05-09" , "sessionCount" : 145 , "sessionCountRate" : 1.3}, 
											{"showDate" : "2015-05-10" , "sessionCount" : 160 , "sessionCountRate" : 1.2},
											{"showDate" : "2015-05-11" , "sessionCount" : 180 , "sessionCountRate" : 1.1},
											{"showDate" : "2015-05-12" , "sessionCount" : 190 , "sessionCountRate" : 1.0}] 
								} ]
							};
			var tempData1 = { "ver" : "1.0" , "ret" : true , "errcode" : 0 , "errmsg" : "" , "cinemaId" : 486,  
							"data" : [
								{ "filmId" : 2, "filmName" : "何以笙箫默", 
