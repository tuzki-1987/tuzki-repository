var createSchDateInCookie = getCookie("createSchDate");	// 获取生成排期时选择的日期值
...
var showSdlHandler = {
	_colligateHandler:function(){
		var crtLgnCinemaName = getCookie("sdleCinemaNickName");
		if (crtLgnCinemaName != "") {
	...
	_showInfoByScheduleDelay:function(){
		...
		removeCookie("createSchDate");	// 移除cookie中的生成排期时所选的日期值
	},
		...
		$('#adjustId').on('click', function(){
			if(emptySchData == 1){
				...
				$.layer({
				    ...
				    fadeIn: 1000,
				    area: ['90%', '1000px'],
				    page: {url: '/schedule/entgroup-schedule/adjust_schedule/adjust_schedule.html'}
				}); 
			}else{
