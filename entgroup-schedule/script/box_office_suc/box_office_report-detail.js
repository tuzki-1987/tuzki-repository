 ...
 * @return
 * */
function showDetail(thisTitle, current) {
	var current = 0;
	loadBoxOfficeDetail(current, 5, 'detailIndex');
 ...
 * @return
 * */
function loadBoxOfficeDetail(current, offset, obj) {
...
function showPageByData(arr_data, submit_flag) {
	...
	if(submit_flag == 1){	// 已提交
		...
		html_submitBtn = "<div id='submitBtnBox' style='margin-top: 20px; text-align: center;'>"
							+"<input type='button' id='submitBtn' class='submitBtn' name='submitBtn' value='提交' onclick=\"submitDetail();\" />"
						+"</div>";
	}
