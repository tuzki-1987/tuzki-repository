...
function loadBoxOfficeReport(){
	$("#temptime1").val(getCurDate());
	$("#temptime2").val(getCurDate());
...
function upScheDetail(obj){
	...
	if(cinemaId == "") {
		tipMsg_Single("boxOfficeModel", 0, "登录状态已失效，请重新登录", 0, '', '');
				...
				$.ajaxFileUpload({
			        ...
			        success : function(data, status){
			        	...
			        	if(result.ret){
			        		...
			        		var detailTitleHTML = "<a id='show_detail_btn' href='javascript:;' title='" + detailTitle + "' onclick='showDetail(this.title, 1);'>"+detailTitle+"</a>";
