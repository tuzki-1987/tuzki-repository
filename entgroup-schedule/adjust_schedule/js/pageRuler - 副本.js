...
var crtLgnCinemaId = getCookie("cinemaId");
...
var moveOption = {
	...
	_moveFilm:function(){
		$('.film').draggable({
				...
				drag:function(e,ui){
					...
					h = ("0000000000"+h).substr(-2);
					m = ("0000000000"+m).substr(-2);
					...
					eh = ("0000000000"+eh).substr(-2);
					em = ("0000000000"+em).substr(-2);
	...
	_goldCount:function(){
		...
		var data_json = {};	// 统计每部影片的黄金场次总计
		for (var i = 0; i < liObj.length; i++) {
			var curAftnNum = 0,curNigNum = 0;
		...
		var trHtml = "<tr class='remove_only'><td>"+$("#userInfoCinema").text()+"</td><td>"+commn_json.normalNum+"</td><td>"
		...
		var goldTrHtml = "";
		var goldIndex = 0;	// 
		var goldTotalTrHtml ="<td rowspan='" + rowspanNum + "' style='vertical-align:middle;'>"+commn_json.goldNum+"</td>";
		$.each(data_json, function(index, val) {
			goldTrHtml += "<tr id='goldTr_" + goldIndex + "'>";
			goldTrHtml += "<td colspan='2'>"+val.name+"</td><td colspan='2'>"+val.filmGoldNum+"</td>" + goldTotalTrHtml;
			goldTrHtml += "</tr>";
		...
		// 重新放置黄金场统计TD的位置
		var resultArr = sortByArray(sortArr);
		var firstRec = resultArr[0].split("-");
		var firstRecArr = firstRec[0].split("_");
		if(firstRecArr[1]*1 != 0){	// 已排完序的数组中的第一条不是原第一条
			var firstRecTd = $("#" + firstRec[0]).find("td:eq(1)");
			var goldTotalTd = $("#goldTr_0").find("td:eq(2)");
			$(goldTotalTd).clone().insertAfter(firstRecTd);	// 将黄金场统计TD的html副本复制到已重排后的第一条记录之后
			$(goldTotalTd).remove();	// 移除原黄金场统计TD
		}
...
function initPage(){
	...
	$("#filmNameForSch").text($("#userInfoCinema").text());	// 设置插件标题的影院名字
