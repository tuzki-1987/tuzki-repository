...
jQuery.extend({
			...
			uploadHttpData : function(r, type) {
				...
				if (type == "json") {
					//eval( "data = " + data );  
					// r.responseText : <pre style="...">{ "ver" : "1.0" , "ret" : true , "errcode" : 0 , "errmsg" : ""}</pre>
					// r.responseText[IE 8] : <PRE style="...">{ "ver" : "1.0" , "ret" : true , "errcode" : 0 , "errmsg" : ""}</PRE>
					if (data.indexOf('<pre') != -1 || data.indexOf('<PRE') != -1) {	// 2015-04-22 处理返回的数据，去掉多余的部分，获取json字符串.大写标签为IE(8)返回格式
						data = data.substring(data.indexOf("{"), data.lastIndexOf("}") + 1);
					}
//					eval("data = \" " + data + " \" ");	// 2015-04-22 注释此处，否则（无论成功失败）一直执行error
				}
