...
function submitDetail() {
	var cinemaId = cinemaObj.id, showDate = $("#show_detail_btn").attr("data");
	...
	if(cinemaId == "") {
		...
		if(failedMatch > 0){
			...
			// 直接生成对比结果
			becomeCompareResultByDirect(cinemaId, cinemaObj.platFormId, showDate);
		}
	}
}
...
function showPageByData(arr_data, submit_flag) {
	...
	var html_submit_btn = "<span class='dc-quick-sub'><button class='btn btn-info btn-sub' onclick='submitDetail();'>提交</button></span>";
	// 组合标题信息
	var html_dc_title = "<p id='dc_title' class='dc-title' onclick='titleCtrlFunc(this);'>" + html_title + html_title_1 + html_title_2 + html_title_3 + "</p>" + html_submit_btn;
