				...
				$.ajaxFileUpload({
			        ...
			        success : function(data, status){
			        	...
			        	if(result.ret){
			        		...
			        		// 不可点击
			        		var detailTitleHTML = "<span id='show_detail_btn' class='show-detail-name' title='" + detailTitle + "' data='" + result.showDate + "'>【" + result.cinemaName + "】&nbsp;" + detailTitle + "</span>";
			        		$("#detailIndex").append(detailTitleHTML);
			        		$(".detailIndex").find(".show-detail-name").fadeIn(1000);
			        		...
			        		// 加载文件明细
			        		setTimeout(function() {
			        			showDetail(detailTitle, 1, result.showDate);
			        		}, 300);
			        	}else{
